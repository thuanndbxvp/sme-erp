...
# Lưu ý: Nên dùng logrotate cho /var/log/db_backup.log để tránh đầy ổ cứng.

set -e # Dừng script ngay nếu có lỗi
set -o pipefail # Đảm bảo bắt lỗi của pg_dump khi dùng pipe

# Cấu hình
# [BẢO MẬT] Không hardcode password vào DB_URL. Hãy dùng ~/.pgpass hoặc truyền qua PGPASSWORD.
export PGPASSWORD="your_secure_password"
DB_URL="postgresql://user@localhost:5432/sme_erp"
...
const openingBalanceSchema = z.object({
  cashBalances: z.array(z.any()), // Thay z.any() bằng schema chi tiết
  arBalances: z.array(z.any()),
  apBalances: z.array(z.any()),
  inventoryBalances: z.array(z.any()),
});
...
export async function POST(req: Request) {
  ...
  // [QUAN TRỌNG] Chống chạy nhiều lần (Idempotency)
  const isInitialized = await prisma.systemSetting.findUnique({ where: { key: "IS_OPENING_BALANCES_DONE" } });
  if (isInitialized?.value === "true") {
    return NextResponse.json({ error: "Dữ liệu đầu kỳ đã được khởi tạo, không thể chạy lại." }, { status: 400 });
  }

  const body = await req.json();
  const validatedData = openingBalanceSchema.parse(body);
  const { cashBalances, arBalances, apBalances, inventoryBalances } = validatedData;
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      // Lưu ý: Thay vì for...of, dùng createMany sẽ tránh quá tải/timeout nếu file excel lớn
      if (apBalances.length > 0) {
        await tx.invoice.createMany({
          data: apBalances.map((ap: any) => ({
...
export async function middleware(request: NextRequest) {
  ...
  // [BẢO MẬT] Trên môi trường Production (HTTPS), cookie đổi thành __Secure-authjs.session-token
  ...
  if (request.nextUrl.pathname === "/api/auth/callback/credentials") {
    // [BẢO MẬT] Lấy đúng IP thật nếu đứng sau Nginx/Cloudflare
...
// (Với App Router, tham số đường dẫn thường nằm trong params.id)
