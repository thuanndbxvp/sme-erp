# MSEW: Micro-Step Execution Workflow (Fix RBAC Role)

> **Gửi Tầng 2 (Coder):** Thao tác cẩn thận 2 bước này để lấy dữ liệu `role` tươi từ DB thay vì đọc từ Session ảo.

## Bước 1: Sửa file Layout
Mở file `src/app/(dashboard)/layout.tsx`. Cập nhật toàn bộ nội dung thành:
...
export default async function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      <main className="flex-1 ml-[var(--sidebar-width)] bg-slate-50 min-h-screen">
        {children}
      </main>
    </div>
  );
...
## Bước 2: Sửa file Dashboard
Mở file `src/app/(dashboard)/page.tsx`. Thay thế dòng lấy `userRole` và import `prisma`:
...
Sau khi sửa xong, lưu file. Mở màn hình lên sếp sẽ thấy Full Menu và Lãi Gộp xuất hiện ngay tắp lự!
