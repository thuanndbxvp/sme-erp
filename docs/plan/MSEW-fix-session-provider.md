# MSEW: Micro-Step Execution Workflow (Fix Session Provider Error)

> **Gửi Tầng 2 (Coder):** Thao tác chính xác 2 bước này để loại bỏ hoàn toàn lỗi Prerender của Next.js khi Build.

## Bước 1: Sửa file Sidebar để nhận Props tĩnh
Mở file `src/components/layout/Sidebar.tsx`. 
- **Xóa** dòng import `useSession`.
- Cập nhật định nghĩa component để nhận tham số.

Tìm đến đoạn `export function Sidebar() {` và sửa lại thành:
...
export function Sidebar({ userRole, userName }: { userRole: string; userName: string }) {
  ...
  const MENU: MenuGroup[] = [
    // ... (Giữ nguyên các mảng menu hiện tại của bạn)
    {
      title: "Kinh doanh",
      items: [
...
// Giữ nguyên SectionLabel và NavItem
...
## Bước 2: Sửa Layout truyền dữ liệu xuống
Mở file `src/app/(dashboard)/layout.tsx`. Cập nhật để nó gọi `auth()` ở Server và truyền biến xuống `<Sidebar>`:

```tsx
import type { ReactNode } from "react";
import { Sidebar } from "@/components/layout/Sidebar";
import { auth } from "@/lib/auth"; // Nhớ import auth

export default async function DashboardLayout({ children }: { children: ReactNode }) {
  // Lấy session ngay tại Server, tốc độ siêu nhanh
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      {/* Truyền role và name xuống Sidebar */}
...
Sau khi sửa xong 2 file này, chạy lại lệnh `npm run build` là màn hình console sẽ hiện `✓ Compiled successfully` xanh rờn.
